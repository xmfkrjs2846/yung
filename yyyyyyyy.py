 ...
 if message.author != client.user:
   ...
   if talk[0] == 0:
    ...
    if message.content.startswith('성규') or message.content.startswith('슬기') or message.content.startswith('성구'):
        ...
        msg = "{0.author.mention}이가 부르고있어 어서 대답해주는게 좋을 것 같은데?".format(message)
        await client.send_message(me, "{}".format(msg))
        await client.send_message(message.channel, "{} 530만원 쓴 음머어~ 찾아요!!".format(myid))
    ...
    if message.content.startswith("재획"):
        ...
        if msg.content == '1' or msg.content == '1번':
            ...
            a = await client.send_message(message.channel, "{0.author.mention}아 경험치 쿠폰을 곧 먹어야해 4/2".format(message))
            await asyncio.sleep(5)
            await client.delete_message(a)
            await asyncio.sleep(1798)
            b = await client.send_message(message.channel, "{0.author.mention}아 경험치 쿠폰을 곧 먹어야해 4/3".format(message))
            await asyncio.sleep(5)
            await client.delete_message(b)
            await asyncio.sleep(1798)
            c = await client.send_message(message.channel, "{0.author.mention}아 경험치 쿠폰을 곧 먹어야해 4/4".format(message))
            await asyncio.sleep(5)
            await client.delete_message(c)
            await asyncio.sleep(1798)
            axc = await client.send_message(message.channel, "수고했어~")
